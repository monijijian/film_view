...
class FindMaoyanSerializer(serializers.Serializer):
    film_id = serializers.IntegerField()
class FindMaoyan(generics.GenericAPIView):
    authentication_classes = (JWTAuthentication,)
    serializer_class = AddFriendSerializer
    def post(self,request):
        request_log(request)
        try:
            json_data = {"message": "okk", "errorCode": 0, "data": {}}
            maoyan = Maoyan()
            datas = maoyan.run()
            print(datas)
...
class FindDoubanSerializer(serializers.Serializer):
    film_id = serializers.IntegerField()
class FindDouban(generics.GenericAPIView):
    authentication_classes = (JWTAuthentication,)
    serializer_class = AddFriendSerializer
    def post(self,request):
        request_log(request)
